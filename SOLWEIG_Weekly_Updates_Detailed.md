# SOLWEIG v2022a — Weekly Progress Updates (Detailed Narrative)
...
**Note:** This is the **narrative-expanded copy** of `SOLWEIG_Weekly_Updates.md`. It keeps all technical content (tables, Mermaid diagrams, image links, and code) and expands thin bullet-only Goals / Tasks / Deliverables / Notes / Plan sections into clear prose explaining what was done, why it mattered, and what it proved. A condensed reference also exists as `SOLWEIG_Weekly_Analysis.md`.

---

## Project objectives (12-week)

This twelve-week project moves from understanding SOLWEIG as a finished scientific tool to being able to build a similar QGIS plugin ourselves. The early weeks map how the UMEP SOLWEIG package is structured, what it consumes and produces, and how radiation physics yields Mean Radiant Temperature (Tmrt). Mid-project weeks deepen that physics reading and exercise a real end-to-end run. Later weeks turn that knowledge into practice: a loadable plugin skeleton, SOLWEIG-style input handling, threaded output writing, and progressively more physical computation blocks.
...
Week 1 established orientation before any deep physics or plugin coding. The aim was to know where SOLWEIG lives inside UMEP, what scientific question it answers, and how its source files are layered so later weeks could zoom into specific modules without getting lost.
...
We opened the UMEP repository, located the `SOLWEIG/` package and shared `Utilities/`, and confirmed how the tool appears in the QGIS menu under Outdoor Thermal Comfort. Reading the root manifest and entry point clarified that SOLWEIG is not a standalone plugin but a tool registered through UMEP. Cataloguing every source file into GUI, compute, and shared-utility layers produced a reusable map for Weeks 2–4.
...
By the end of Week 1 the workspace was usable, the SOLWEIG source was located, and the team had a shared mental model of the plugin. The deliverables below are the artifacts that later weeks cite when explaining I/O, physics modules, and our own plugin contract.
...
Two orientation issues shaped how we paced the rest of the project. First, SOLWEIG does not ship as its own QGIS plugin: menu registration goes through `UMEP.py`, which matters when we later build a standalone package with its own `metadata.txt`. Second, `Solweig_2022a_calc.py` and the flux helpers are dense enough that a full physics read in Week 1 would have blocked the architecture work, so detailed flux math was deferred to Week 3.
...
Week 2 turns the Week-1 map into a precise specification: every raster and met column, every output product, every default, and the runtime sequence that stitches them together. That specification is what lets us validate our own plugin against SOLWEIG later.
...
Week 2 closed the documentation gap between “we know the files” and “we know exactly what a run consumes and produces.” The goals below force a complete I/O and defaults record so physics reading (Week 3) and hands-on runs (Week 4) rest on verified contracts rather than guesses.
...
Work focused on reading the orchestration path end to end: how `solweig.py` loads and validates layers, how `solweigworker.py` loops timesteps, and what dictionaries define met inputs and POI outputs. Defaults were taken from both the Qt UI file and hard-coded constants so our later plugin spinboxes could match SOLWEIG’s baseline behavior. Menu registration notes closed the packaging picture started in Week 1.
...
Week 2’s deliverable is this self-contained specification: enough detail that a teammate could prepare inputs, predict which outputs appear, and explain why a parameter change matters—without reopening the source. The sections listed below are the durable reference used when designing our own plugin’s validation and defaults.
...
Documentation honesty mattered here. Unused met-file column names follow the broader UMEP/SUEWS forcing layout and were labeled accordingly, while only the eleven columns SOLWEIG actually reads were treated as code-confirmed. Detailed flux assembly was left for Week 3 so this week’s I/O and execution diagrams stayed accurate and finishable.
...
With the I/O contract documented, Week 3 goes inside the core calc: how directional shortwave and longwave fluxes are assembled into `Sstr` and then `Tmrt`, and how shadows and the Perez sky option change those fluxes. Parallel preparation of sample data sets up the Week 4 hands-on run.
...
Week 3 connects the governing Tmrt equation from Week 1 to the concrete Python modules that fill each term. Without tracing how K and L fluxes are built per direction, the later plugin’s “first computation” steps would remain arbitrary transforms instead of deliberate approximations of SOLWEIG physics.
...
Each file below owns a piece of the six-direction radiation budget that feeds Tmrt. Ground view factors (`gvf_2018a`) determine how nearby surfaces radiate onto a pixel; Kup/Kside modules assemble reflected and lateral shortwave; Lside/Lcyl do the longwave analogue onto walls and a human-scale cylinder. Shadow helpers decide which cells see the sun; Perez plus sky patches replace isotropic diffuse assumptions when anisotropic sky is enabled; cylindric wedge and TsWaveDelay capture wall-shadow fraction and thermal inertia in surface temperature—both required before longwave fluxes are realistic.
...
This diagram is the Week 3 reading map: sun position and shadows feed shortwave; surface temperature and sky view factors feed longwave; both combine into `Sstr` and then Tmrt. Annotating it with equations from the modules in §3.2 is the concrete proof that we understand how each file contributes to the final map.
...
The Week 3 write-up should make the flux path teachable: per-direction K and L notes tied to file names, plus a short comparison of isotropic versus Perez anisotropic sky so we know when the extra `shadowmats.npz` cost is justified. Those notes become the checklist when we later decide which physics to approximate first in our plugin.
...
The main risk is time spent re-deriving textbook radiation formulas instead of mapping how SOLWEIG implements them. We will cite the key papers (Perez 1993, Martin & Berdahl 1984, Reindl 1990) and focus on code paths, defaults, and day/night branches that affect Tmrt maps.
...
Week 4 moves from reading physics to exercising it: assemble a consistent raster/met set with UMEP preprocessors, run SOLWEIG, and interpret Tmrt and POI outputs against the Week 2 specification.
...
A hands-on run proves the Week 1–3 analysis against real data. Success looks like a finished input stack (DSM through SVF and met), a completed SOLWEIG run without grid-mismatch failures, a readable Tmrt map on the canvas, and a POI time-series whose columns match the Week 2 dictionary. That experience also shows which preprocessing steps are the usual failure points before we reproduce them in our plugin.
...
Tasks follow the real UMEP pipeline order so each product is available before SOLWEIG starts. A single-timestep run validates wiring quickly; a diurnal run exercises day/night naming and averages. Loading the styled average Tmrt and checking a POI file plus RunInfo confirms outputs and reproducibility against the settings we chose.
...
The chart below is the end-to-end path Week 4 must complete. Preprocessors feed SOLWEIG; SOLWEIG must emit Tmrt rasters and optional POI comfort series. Completing this pipeline is the practical test that our documented inputs and menu integration match how the tool is actually used.
...
Evidence of a successful Week 4 is visual and procedural: screenshots of Tmrt and a POI series, plus a reusable checklist so the run can be repeated or demonstrated without re-discovering preprocessor order.
...
The dominant practical risk is misaligned rasters. SOLWEIG aborts when extent or resolution differs from the DSM; most failed first runs in practice come from that hard rule rather than from physics bugs. Week 4 therefore treats grid matching as a first-class validation step—the same rule we later encode in our plugin.

- Input rasters must share identical extent/resolution — most likely source of run failures.

## 4.6 Plan for Week 5

After proving we can run SOLWEIG, Week 5 switches to packaging: a minimal loadable QGIS plugin that implements the same `classFactory` / `initGui` / `unload` / `run` contract so we can grow features toward SOLWEIG-like behavior.
...
Week 5 reproduces the minimal QGIS plugin contract that SOLWEIG (via UMEP) relies on: a `metadata.txt` manifest, `classFactory(iface)` entry point, and a plugin class with `initGui`, `unload`, and `run`. Success is not physics—it is proving our package loads in QGIS 4, appears under Plugins, and can talk to the project (e.g. count layers).

- Build a loadable plugin that registers a menu item and opens a dialog.

## 5.2 Tasks completed

We built `hello_qgis_plugin/` with the three required files and implemented the same factory/lifecycle methods UMEP uses. The first dialog was written in code (Qt Designer `.ui` deferred) so we could validate install and menu wiring quickly. Installing into the QGIS **4** profile folder—not QGIS3—and testing on Solweig_latest proved the contract: the plugin loaded, the menu opened, and `QgsProject` reported 21 layers.
...
The practical packaging lesson was versioned profiles: QGIS 3 and QGIS 4 keep separate plugin directories. Copying into the wrong folder looks like a broken plugin even when the code is fine. Marking `experimental=True` and enabling experimental plugins in the manager was required for discovery. Editable source remains under the GA Summer `hello_qgis_plugin` folder and must be re-copied (or symlinked) into the active profile after changes.
...
With the skeleton loading, Week 6 adds SOLWEIG-style data intake: layer combo boxes, GDAL→NumPy reads, geo metadata, and grid-match validation—still without full radiation physics.
...
Week 6 proves our plugin can consume GIS data the way SOLWEIG does before any Worker thread exists. Goals are layer selection, a GDAL read path into NumPy with scale/lat-lon/NoData handling, and validation that two rasters share extent and resolution—the hard rule that most often stops a real SOLWEIG run.

- Add raster/point layer pickers, read a raster into NumPy, and validate it.

## 6.2 Tasks completed

Implementation followed SOLWEIG’s input-handling patterns from `solweig.py`, deliberately limited to a minimal proof set (two rasters + one POI layer) rather than the full eight-plus combo boxes. Validation mirrors SOLWEIG: raise negatives after NoData handling, derive scale from pixel size, reproject a corner to WGS84, and compare grids before claiming readiness for a run.
...
Keeping editable source separate from the QGIS profile install path remains essential: development happens in the GA Summer folder; QGIS 4.0.3 only sees the copy under the QGIS4 profiles plugins directory. After Week 6 changes (v0.2), both paths must stay in sync for screenshots and demos on Solweig_latest.
...
Week 7’s purpose is the output pipeline SOLWEIG uses to stay interactive: a background `Worker` on a `QThread`, progress and cancel signals, GDAL GeoTIFF write, and auto-load into the project. Physics remains minimal on purpose—the week proves the threading and write path so later computation can plug in without freezing QGIS.

### 6.8.1 Code changes planned

The code list below implements that SOLWEIG-like threading and output contract: run off the UI thread, report progress, allow cancel, write a GeoTIFF, and reload it for visual confirmation.
...
UI work makes the threaded run operable: choose where outputs land, start/stop execution, and keep the summary panel as a live log of status, paths, and timing.
...
Validation gates protect the Worker: no run without a primary raster, no write into a non-writable folder, and no proceed if an optional second raster fails the extent/resolution match.
...
Acceptance for Week 7 is evidence that the pipeline works: a versioned plugin, one GeoTIFF on disk and on the map, screenshots of progress and success, and a short log covering both a happy path and a blocked validation case.
...
Week 8 shifts from “the write path works” to “cells compute something thermal.” The baseline formula is intentionally simple: it proves end-to-end physics wiring (parameters → Worker → GeoTIFF) and repeatable run folders, while documenting assumptions so later weeks can replace the placeholder with real sun-driven or flux-based logic without rewriting the pipeline.
...
The baseline equation below is a teaching scaffold, not SOLWEIG parity. Its purpose is to show that raster values and scalar controls flow through the Worker into a map whose statistics respond to `Ta`, `Wind`, and a Kdown proxy—so architecture and correctness can be tested before importing multi-flux radiation code.
...
UI additions expose the baseline formula’s knobs and make runs reproducible: named/timestamped output folders and an explicit “Baseline v0.1” mode label so screenshots and RunInfo files stay unambiguous.
...
Automated unit tests cover the pure-Python Week 9 modules; the matrix below is the remaining QGIS-side proof that time of day, radG, and SVF actually change the map—and that missing or mismatched SVF still blocks the run.
...
Week 9 deliverables combine code, tests, and run metadata. Manual QGIS screenshots and a filled test matrix remain the final evidence pack before claiming full UI verification.
...
Overall, the project has completed the analysis arc (how SOLWEIG is structured, what it reads and writes, and how it integrates with UMEP/QGIS) and established a working plugin foundation through input handling, threaded GeoTIFF output, a baseline computation mode, and a first physically motivated Kdown driven by sun position and SVF. Weeks 3–4 remain the planned physics deep dive and hands-on SOLWEIG run; Weeks 10–12 still need shadow casting, a met-file timestep loop, packaging polish, and documentation to approach fuller SOLWEIG-like behavior.
